...
public class FreeChatLauncher extends JFrame {
    ...
    public FreeChatLauncher() {
        try {
            // Automatically changes the UI to match the specific operating system's window UI.
        ...
        // Upon joining a server, close the options window and show the chat client
        ...
        try {
            ...
            if (port == -1) {
                JOptionPane.showMessageDialog(
                        this,
                        "Could not find an available port after several attempts.",
                        "Port Error",
            ...
            String className = "FreechatServer";

            ProcessBuilder builder = new ProcessBuilder( // Used to create a process (consisting of multiple threads: clients + server)
                    javaBin,
                    "-cp",
                    classpath,
                    className,
            ...
            // Proceed to the chat client GUI after connecting to the server
            dispose();
            SwingUtilities.invokeLater(() -> {
                ChatClientGUI client = new ChatClientGUI();

                // Set the pre-filled values directly for the host only
    ...
    public static int findRandomAvailablePort() {
        ...
        for (int attempt = 0; attempt < maxAttempts; attempt++) {
            int port = random.nextInt(5000, 65535); // Port between 5000 and 65535, out of range is used for other network related services
            try (ServerSocket _ = new ServerSocket(port)) {
                return port;
            } catch (IOException e) {
                System.out.println("Could not find available port, try again later" );
            }
        }
        return -1;
    }
     ...
     // Gets the local IPv4 address of the machine on the network instead of Localhost/127.0.0.1
    private String getLocalIPAddress() {
        ...
        } catch (Exception e) { //In the event fetching the local IP address fails, just return the local host IP
            System.err.println("Error getting local IP address");
            return "127.0.0.1";
        }
    }
